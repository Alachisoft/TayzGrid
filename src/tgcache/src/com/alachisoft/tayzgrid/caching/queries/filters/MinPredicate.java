...
public class MinPredicate extends AggregateFunctionPredicate {
    ...
    public void Execute(QueryContext queryContext, Predicate nextPredicate) throws StateTransferException, Exception {
        ...
        if (type != null) {
            if ((type != java.util.Date.class) && (type != String.class) && (type != Character.class)) {
                if (min != null) {
                    super.SetResult(queryContext, AggregateFunctionType.MIN, new BigDecimal(((Number) min).doubleValue()));
                    return;
                }
            }
        }
        super.SetResult(queryContext, AggregateFunctionType.MIN, min);
    }
