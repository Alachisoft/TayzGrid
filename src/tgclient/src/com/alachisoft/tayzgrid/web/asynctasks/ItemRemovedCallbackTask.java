...
public class ItemRemovedCallbackTask implements AsyncProcessor.IAsyncTask {
        ...
        public ItemRemovedCallbackTask(Broker parent, Object key, short callBackId, Object value, CacheItemRemovedReason reason, BitSet flag, boolean notifyAsync, EventCacheItem item,EventDataFilter dataFilter)
        {
            this._parent = parent;
            this._key = key;
        ...
        public void Process()
        {
            try
            {
                if (_parent != null && _parent._cache.getAsyncEventsListener() != null)
                {
                    ...
                    _stats.BeginSample();
                    _parent._cache.getEventsListener().OnCustomRemoveCallback(_callBackId, _key, _value, _reason, _flag, _notifyAsync, _item,_dataFilter);
            ...
            catch (Exception ex)
            {
                if (_parent != null && _parent.getLogger()!=null && _parent.getLogger().getIsErrorLogsEnabled()) 
                    _parent.getLogger().getCacheLog().Error("Item Remove CallBack Task.Process", ex.getMessage());
            }
        }
