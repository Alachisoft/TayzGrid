...
public class ItemAddedEventTask implements AsyncProcessor.IAsyncTask {
    ...
    public ItemAddedEventTask(Broker parent, Object key, boolean notifyAsync, EventCacheItem item, BitSet flag) {
        this._parent = parent;
        this._key = key;
         ...
         try
            {
                if (_parent != null)
                {
                    ...
                    _stats.BeginSample();
                    _parent._cache.getEventsListener().OnItemAdded(_key, _notifyAsync, _item, _flag);
            ...
            catch (Exception ex)
            {
                if (_parent != null && _parent.getLogger()!=null && _parent.getLogger().getIsErrorLogsEnabled()) 
                    _parent.getLogger().getCacheLog().Error("Item Added Task.Process", ex.getMessage());
            }
    }
}
