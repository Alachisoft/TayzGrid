...
import com.alachisoft.tayzgrid.common.protobuf.GetNextChunkResponseProtocol;
...
public class GetNextChunkCommand extends CommandBase
{
    ...
    public void ExecuteCommand(ClientManager clientManager, com.alachisoft.tayzgrid.common.protobuf.CommandProtocol.Command command)
    {
        ...
        try
        {
            ...
            if (!clientManager.getEnumerationPointers().containsKey(cmdInfo.Pointer.getId()))
            {
                clientManager.getEnumerationPointers().put(cmdInfo.Pointer.getId(), cmdInfo.Pointer);
            }
            else
            {
                clientManager.getEnumerationPointers().put(cmdInfo.Pointer.getId(), cmdInfo.Pointer);
            ...
            GetNextChunkResponseProtocol.GetNextChunkResponse.Builder getNextChunkResponse = GetNextChunkResponseProtocol.GetNextChunkResponse.newBuilder();


            if (nextChunk.getData() != null)
            {
                for(Object key : nextChunk.getData())
                    getNextChunkResponse.addKeys(CacheKeyUtil.toByteString(key, nCache.getCacheId()));
            }

            getNextChunkResponse.setEnumerationPointer(EnumerationPointerConversionUtil.ConvertToProtobufEnumerationPointer(nextChunk.getPointer()));


            ResponseProtocol.Response response = ResponseProtocol.Response.newBuilder().setGetNextChunkResponse(getNextChunkResponse.build())
                    .setRequestId(Long.parseLong(cmdInfo.RequestId))
                    .setIntendedRecipient(cmdInfo.OperationContext.GetValueByField(OperationContextFieldName.IntendedRecipient).toString())
                    .setResponseType(ResponseProtocol.Response.Type.GET_NEXT_CHUNK).build();
    ...
    {
        CommandInfo cmdInfo = new CommandInfo();
        GetNextChunkCommandProtocol.GetNextChunkCommand getNextChunkCommand = command.getGetNextChunkCommand();
        cmdInfo.RequestId = (new Long(command.getRequestID())).toString();
        cmdInfo.Pointer = EnumerationPointerConversionUtil.GetFromProtobufEnumerationPointer(getNextChunkCommand.getEnumerationPointer());
