...
public class ReadWriteTayzGridRegionAccessStrategy extends TayzGridRegionAccessStrategy {

    private int _nextLockId = Integer.MIN_VALUE;
    private final CacheDataDescription _cacheDataDescription;

    public ReadWriteTayzGridRegionAccessStrategy(TayzGridTransactionalDataRegion region) {
        super(region);
        _cacheDataDescription = region.getCacheDataDescription();
    }
    ...
    public Object get(Object key, long txTimestamp) throws CacheException {
        Lockable lockable = (Lockable) this._region.getCache().get(key);
    ...
    public SoftLock lockItem(Object key, Object version) throws CacheException {
        try {
            ...
            long timeout = _region.getCache().nextTimestamp() + _region.getCache().getTimeout();
            Lock lock = lockable == null ? new Lock(timeout, nextLockId(), version) : lockable.lock(timeout, nextLockId());

            _region.getCache().put(key, lock);
            return lock;
        } finally {
    ...
    public boolean putFromLoad(
        ...
        try {
            _region.getCache().lock(key);
            Lockable lockable = (Lockable) _region.getCache().get(key);
            boolean puttable = (lockable == null) || (lockable.isPuttable(txTimestamp, version, _cacheDataDescription.getVersionComparator()));
            boolean bool1;
            if (puttable) {
    ...
    boolean isUnlockable(SoftLock clientLock, Lockable myLock)
    ...
    void handleLockExpiry(Object key) throws CacheException {
        long ts = _region.getCache().nextTimestamp() + _region.getCache().getTimeout();
    ...
    int nextLockId() {
        if (this._nextLockId == Integer.MAX_VALUE) {
            this._nextLockId = Integer.MIN_VALUE;
        }
        return this._nextLockId++;
    }

    void decrementLock(Object key, Lock lock) {
        lock.unlock(_region.getCache().nextTimestamp());
        _region.getCache().put(key, lock);
    }
}
