...
public class MemcachedClient {
        ...
        IMemcachedProvider memcachedProvider;
	// logger
	 ...
	 * accepts a passed in ClassLoader.
	 * 
	...
	/** 
	 * Creates a new instance of MemCacheClient but
	 * accepts a passed in ClassLoader and a passed
	 * in ErrorHandler.
	 * 
	 * @param classLoader ClassLoader object.
	...
	/** 
	 * Creates a new instance of MemCacheClient but
	 * accepts a passed in ClassLoader, ErrorHandler,
	 * and SockIOPool name.
	 * 
	 * @param classLoader ClassLoader object.
	 * @param errorHandler ErrorHandler object.
	 ...
	 *
	 * This enables compression and sets compression threshold to 15 KB.
	 */
	private void init() {
	...
	public boolean delete( String key, Integer hashCode, Date expiry ) {
		...
		try {
		 ...
		 if(memcachedProvider==null)
                {
                    log.error( "++++ failed to get SockIO obj for: " + "localhost" );
                    return false;
                }
                long seconds=0;
		if ( expiry != null )
			seconds=expiry.getTime() / 1000;
	...
	private boolean set( String cmdname, String key, Object value, Date expiry, Integer hashCode, boolean asString ) {
		...
		try {
                    ...
                    {
                        case "set":
                            opResult = memcachedProvider.set(key, flags, (expiry.getTime() / 1000), val);
                            break;
                        case "add":
                            opResult = memcachedProvider.add(key, flags, (expiry.getTime() / 1000), val);
                            break;
                        case "replace":
                            opResult = memcachedProvider.replace(key, flags, (expiry.getTime() / 1000), val);
                            break;
                    }
                String line = "";
                if (cmdname.equals("replace") && opResult.getReturnResult() == Result.ITEM_NOT_FOUND)
                    line = "NOT_STORED";
                else
                    switch (opResult.getReturnResult())
                    {
		...
		try {
			...
			return -1;
                ...
                if(memcachedProvider==null)
                {
                 ...
                 if (inc < 0)
                {
                   log.error("CLIENT_ERROR cannot increment or decrement non-numeric value");
                   return -1;
                }
		try {
                ...
                {
                    case "incr":
                        opResult = memcachedProvider.increment(key, (long)inc, null, 0, 0);
                        break;
                    case "decr":
                        opResult = memcachedProvider.decrement(key, (long)inc, null, 0, 0);
                        break;
                }
                // get result back
                String line = "";

                if (opResult.getReturnResult() == Result.SUCCESS)
                    line = opResult.getMutateResult()+"";
                else
                    if (opResult.getReturnResult() == Result.ITEM_NOT_FOUND)
                        line = "NOT_FOUND";
                    else
                        if(opResult.getReturnResult()==Result.ITEM_TYPE_MISMATCHED)
	...
	public Object get( String key, Integer hashCode, boolean asString ) {
		...
		try {
			...
			// ready object
			Object o = null;
                     for(GetOpResult resultEntry :opResult)
                        {
				String line = "";
...
//				if ( line.startsWith( VALUE ) ) {
					int flag      = (int)resultEntry.getFlag();
					...
					byte[] buf = (byte[])resultEntry.getReturnValue();
	...
	public Map<String,Object> getMulti( String[] keys, Integer[] hashCodes, boolean asString ) {
		...
		Map<String,Object> ret =
			new HashMap<String,Object>( keys.length );
                ...
                try {
                        String[] keysArray= new String[cleanKeys.size()];
                        cleanKeys.toArray(keysArray );
                        loadMulti( keysArray, ret, asString );
                    }
                    catch ( Exception e ) {
                            // shouldn't happen; we have all the data already
                        log.warn( "Caught the aforementioned exception on connection");
                    }
		for ( String key : keys ) {
	 ...
	 */
	private void loadMulti(String[] keys, Map<String,Object> hm, boolean asString ) throws Exception {
	...
	public Map statsItems( String[] servers ) {
		return stats( servers, "items", STATS );
	}
	...
	public Map statsSlabs( String[] servers ) {
		return stats( servers, "slabs", STATS );
	}
	...
	public Map statsCacheDump( String[] servers, int slabNumber, int limit ) {
		return stats( servers, String.format( "cachedump %d %d", slabNumber, limit ), ITEM );
	}
	...
	private Map stats( String[] servers, String command, String lineStart ) {
		...
		Map<String,Map> statsMaps =
			...
			try {
                            OperationResult opResult=new OperationResult();
                            opResult=memcachedProvider.getStatistics(command);


                            // map to hold key value pairs
                            Map<String,String> stats = (Hashtable)opResult.getReturnValue();
//                            for(int i=0;i<servers.length;i++)
//                            {
                                statsMaps.put( "memcached", stats );
//                            }
			}
			catch ( Exception e ) {
